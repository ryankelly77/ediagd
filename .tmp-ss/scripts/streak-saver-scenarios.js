...
const pref = (over = {}) => ({
    ...push_prefs_1.DEFAULT_PUSH_PREF,
    ...over,
});
console.log("\n  THE SOFT-ASK BUDGET\n");
/* Nothing before the app has been useful. An ask on day zero is a stranger
   wanting something. */
check("no ask before the first completed day", (0, push_prefs_1.shouldOfferSoftAsk)({ completions: 0, streak: 0, pref: pref() }), false);
check("the first ask lands on the first completed day", (0, push_prefs_1.shouldOfferSoftAsk)({ completions: 1, streak: 1, pref: pref() }), true);
...
check("no second ask before the streak gives it a reason", (0, push_prefs_1.shouldOfferSoftAsk)({ completions: 3, streak: 3, pref: pref({ softAskCount: 1 }) }), false);
check(`the second ask waits for a streak of ${push_prefs_1.SECOND_ASK_STREAK}`, (0, push_prefs_1.shouldOfferSoftAsk)({
    completions: 5,
    streak: push_prefs_1.SECOND_ASK_STREAK,
    pref: pref({ softAskCount: 1 }),
}), true);
/* THE BUDGET IS TWO. This is the assertion the whole file exists for. */
check("never a third ask, however long the streak", (0, push_prefs_1.shouldOfferSoftAsk)({
    completions: 90,
    streak: 90,
...
check("never asked again after saying yes", (0, push_prefs_1.shouldOfferSoftAsk)({
    completions: 9,
    streak: 9,
...
check("'not now' still leaves the second ask available", (0, push_prefs_1.shouldOfferSoftAsk)({
    completions: 5,
    streak: push_prefs_1.SECOND_ASK_STREAK,
