 ...
 */
/**
 * Rebuild a canonical PEM from whatever shape the key arrived in.
 *
 * ---------------------------------------------------------------------------
 * WHY THIS IS NOT JUST A TRIM
 * ---------------------------------------------------------------------------
 * The first version replaced escaped \n and hoped. It got
 * `error:1E08010C:DECODER routines::unsupported` from OpenSSL, which is not a
 * complaint about the KEY — the key is fine — but about its FORMATTING. PEM is
 * whitespace-significant: the base64 body has to be on its own lines, and a
 * value that loses its newlines somewhere between a text editor, a clipboard
 * and an environment variable is no longer parseable even though every
 * character of the secret is still present.
 *
 * There is no reason to make a person get that exactly right by hand. The
 * information content of a PEM is the label and the base64; everything else is
 * layout, and layout can be regenerated. So this throws the layout away and
 * rebuilds it: pull the label, strip every character that is not base64, re-wrap
 * at 64 columns, reassemble.
 *
 * It accepts a correct PEM, a flattened one, one with escaped newlines, one
