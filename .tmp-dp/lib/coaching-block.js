...
exports.stageForIndex = stageForIndex;
exports.opCodeForBlock = opCodeForBlock;
exports.loadCoachableCodes = loadCoachableCodes;
exports.ensureBlockForToday = ensureBlockForToday;
exports.readOpenBlock = readOpenBlock;
exports.loadBlockDays = loadBlockDays;
 ...
 */
function stageForIndex(completionsInBlock) {
    const i = Math.max(0, Math.floor(completionsInBlock));
    return exports.STAGES[i % exports.STAGES.length];
}
/**
 ...
 */
function opCodeForBlock(coachableCodes, startedOn) {
    if (coachableCodes.length === 0)
        return null;
    const sorted = [...coachableCodes].sort();
    return sorted[(0, daily_1.epochDay)(startedOn) % sorted.length];
}
/**
 ...
 */
async function loadCoachableCodes(client, family) {
 ...
 */
async function ensureBlockForToday(service, userId, rooftopId, today, 
...
async function readOpenBlock(client, userId) {
    const { data, error } = await client
        .from("coaching_block")
        .select("id, family, op_code, tier, started_on, length_days")
        .eq("user_id", userId)
...
async function loadBlockDays(client) {
    ...
    return n > 0 ? n : exports.STAGES.length;
}
