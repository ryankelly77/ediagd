...
exports.CUES_PER_SERVICE = exports.ACK_LABELS = void 0;
exports.dayOfYear = dayOfYear;
exports.epochDay = epochDay;
exports.pickQuoteForSlot = pickQuoteForSlot;
exports.pickQuotesForDay = pickQuotesForDay;
exports.pickQuoteOfDay = pickQuoteOfDay;
exports.pickCoachingCueForBlock = pickCoachingCueForBlock;
exports.pickPitchVideo = pickPitchVideo;
exports.cueTierForRate = cueTierForRate;
exports.ackLabel = ackLabel;
exports.listCuesForServices = listCuesForServices;
exports.pickLifestyleVideo = pickLifestyleVideo;
const playback_1 = require("@/lib/mux/playback");
 ...
 * as before.
 */
function rotationIndex(date, count, offset = 0) {
    if (count <= 0)
        return 0;
    return (epochDay(date) + offset) % count;
}
const CUE_COLUMNS = "id, type, service_family, subcategory, tier, make, model, year_range, title, body, video_url, duration_sec, status, source, created_at, updated_at";
/**
 ...
 */
async function pickByRotation(client, filters, date, offset) {
    const { count, error: countError } = await filters(client.from("content").select("id", { count: "exact", head: true }));
    if (countError || !count)
        return null;
 ...
 */
function voiceDiverseOrder(rows) {
 ...
 */
async function pickQuoteForSlot(client, date, slot, 
/**
...
alsoExclude) {
    ...
    if (error || !pool || pool.length === 0)
        return null;
    const order = voiceDiverseOrder(pool);
 ...
 */
async function pickQuotesForDay(client, date, 
/**
...
videoArtifactId) {
    ...
    const slot2 = await pickQuoteForSlot(client, date, "slot2", slot3?.id ?? null, videoArtifactId ?? null);
    return { slot2, slot3 };
}
/**
 ...
 */
async function pickQuoteOfDay(client, date) {
 ...
 * somewhere to land.
 */
async function pickCoachingCueForBlock(client, date, block) {
    /*
    ...
    const base = (q) => q.eq("type", "cue").eq("status", "published"); // eslint-disable-line @typescript-eslint/no-explicit-any
    if (block.opCode) {
        if (block.stage && block.tier) {
            const hit = await pickByRotation(client, (q) => base(q).eq("op_code", block.opCode).eq("stage", block.stage).eq("tier", block.tier), date, 1);
            if (hit)
                return { cue: hit, matched: "op_code_stage_tier" };
        }
        if (block.stage) {
            const hit = await pickByRotation(client, (q) => base(q).eq("op_code", block.opCode).eq("stage", block.stage), date, 1);
            if (hit)
                return { cue: hit, matched: "op_code_stage" };
        }
        const hit = await pickByRotation(client, (q) => base(q).eq("op_code", block.opCode), date, 1);
        if (hit)
            return { cue: hit, matched: "op_code" };
    }
    ...
    if (block.tier) {
        const hit = await pickByRotation(client, (q) => base(q).eq("service_family", block.family).eq("tier", block.tier), date, 1);
        if (hit)
            return { cue: hit, matched: "family" };
    }
    const anyTier = await pickByRotation(client, (q) => base(q).eq("service_family", block.family), date, 1);
    if (anyTier)
        return { cue: anyTier, matched: "family" };
 ...
 */
async function pickGenericPassage(client, date) {
    return pickByRotation(client, (q) => q
        .eq("type", "cue")
        .eq("tier", "generic")
        .eq("status", "published")
        .is("service_family", null), date, 7);
}
/**
 ...
 */
async function pickPitchVideo(client, date, userId, block) {
    if (!block?.opCode || !block.stage)
        return null;
    const { data: rows } = await client
        .from("content")
        .select("id, title, stage, mux_playback_id, mux_playback_policy, " +
    ...
    return shaped ? { ...shaped, stage: row.stage ?? null } : null;
}
/**
...
function ackLabel(date) {
    return exports.ACK_LABELS[rotationIndex(date, exports.ACK_LABELS.length, 0)];
}
 ...
 */
async function fetchCueIds(client, families) {
    const rows = [];
 ...
 */
async function listCuesForServices(client, date, services, perService = exports.CUES_PER_SERVICE) {
    ...
    if (ids.length === 0)
        return {};
    const batches = await Promise.all(chunk(ids, ID_BATCH).map(async (batch) => {
        const { data } = await client
            .from("content")
 ...
 */
const LIFESTYLE_COLLECTIONS = ["Mindset", "Craft"];
/**
 ...
 */
async function pickLifestyleVideo(client, today, userId) {
    const { data: rows } = await client
        .from("content")
        .select("id, title, collection, mux_playback_id, mux_playback_policy, " +
        "vertical_playback_id, vertical_status, artifact_id")
        .eq("type", "advisor_video")
 ...
 */
async function shapeVideo(client, row, userId) {
    /*
     ...
     * honest one.
     */
    const useVertical = row.vertical_status === "ready" && Boolean(row.vertical_playback_id);
    const tokens = await (0, playback_1.playbackFor)(useVertical
    ...
    if (row.artifact_id) {
        const { data: twin } = await client
            ...
            .maybeSingle();
        quoteText = twin?.body ?? null;
        quoteVoice = twin?.voice ?? null;
    }
    return {
        contentId: row.id,
        title: row.title,
        ...
        orientation: useVertical ? "vertical" : "landscape",
