 ...
 */
class Visit {
  private session: GateRecord | null = null;
  constructor(private readonly server: GateRecord | null = null) {}

  /** What a player mounting right now is handed as `initialMet`. */
  seenByPlayer(): GateRecord | null {
    return creditedGate(this.server, this.session);
  }

  /** A player crossed the threshold (or its failure valve opened). */
  gateOpened(state: { pct: number; error: boolean }) {
    this.session = gateFromWatch(state);
  }

  /** Would this mount make the advisor watch the video again? */
  demandsAWatch(): boolean {
...
/* ---- The reported bug, in order --------------------------------------- */
{
  ...
  check(
    "loop sees the percentage the card measured",
    visit.seenByPlayer()?.pct,
    97
  );
}
...
/* ---- The session record is the newer fact ------------------------------ */
{
  ...
  check(
    "a gate opened in this visit wins over the page-load record",
    visit.seenByPlayer(),
    { pct: 100, error: false }
  );
}
...
/* ---- The failure valve carries too ------------------------------------- */
{
  ...
  check(
    "a broken player writes no percentage",
    visit.seenByPlayer(),
    { pct: null, error: true }
  );
  check(
    "and the second mount does not re-demand the watch",
    visit.demandsAWatch(),
    false
  );
}
