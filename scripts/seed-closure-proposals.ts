...
import { federalHolidays, missingProposals, yearsToSeed } from "../lib/closures";
...
async function main() {
  ...
  const years = yearsToSeed(today);

  console.log(`\n  Seeding ${years.join(" and ")} — ${federalHolidays(years[0]).length} dates a year\n`);
  ...
  const all = (rooftops ?? []) as { id: string; name: string }[];
  if (all.length === 0) {
  ...
  for (let off = 0; ; off += 1000) {
    const { data } = await sb()
      ...
      .range(off, off + 999);
    const page = (data ?? []) as { rooftop_id: string; closed_on: string }[];
    for (const row of page) {
      const set = existing.get(row.rooftop_id) ?? new Set<string>();
  ...
  for (const r of all) {
    const missing = missingProposals(years, existing.get(r.id) ?? []);
    if (missing.length > 0) {
