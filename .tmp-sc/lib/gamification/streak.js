...
exports.storeClosure = storeClosure;
 ...
 */
function storeClosure(date, closures) {
    if (!closures || closures.length === 0)
        return null;
    return closures.find((c) => c.date === date) ?? null;
}
function isStoreClosed(date, closures) {
...
function scheduledOn(date, context = {}) {
    if (!context.schedule)
        return null;
    return (isWorkDay(date, context.schedule) &&
...
function countMissedWorkDays(from, to, context = {}) {
    ...
    for (let i = 1; i <= limit; i++) {
        ...
        if (isStoreClosed(day, context.closures))
            continue;
        missed++;
    }
    return missed;
