...
function run(cmd, argv) {
    return new Promise((resolve, reject) => {
        const c = (0, node_child_process_1.spawn)(cmd, argv, { stdio: "inherit", env: process.env });
        c.on("exit", (code) => (code === 0 ? resolve() : reject(new Error(`${cmd} exited ${code}`))));
    });
}
async function main() {
    if ((0, node_fs_1.existsSync)(LOCK) && !args.includes("--force-unlock")) {
        const held = (0, node_fs_1.readFileSync)(LOCK, "utf8").trim();
        const alive = (() => {
            try {
    ...
    (0, node_fs_1.writeFileSync)(LOCK, String(process.pid));
    const release = () => { try {
    ...
    const plan = load(PLAN, "run slate:plan first — it maps camera-roll names to canonical ones and carries the cut point").plan;
    const ledger = (0, node_fs_1.existsSync)(LEDGER)
        ? JSON.parse((0, node_fs_1.readFileSync)(LEDGER, "utf8"))
        : {};
    ...
    const res = await fetch(`${url}/rest/v1/content?select=id,title,canonical_filename,duration_sec,status,mux_asset_id&canonical_filename=not.is.null`, { headers: { apikey: key, authorization: `Bearer ${key}` } });
    ...
    for (const row of rows) {
        ...
        if (!entry)
            continue; // not from this batch
        const source = entry.file;
        if (ONLY && !row.canonical_filename.toLowerCase().includes(ONLY.toLowerCase()))
            continue;
    ...
    for (const j of jobs) {
        ...
        try {
            ...
            (0, node_fs_1.writeFileSync)(LEDGER, `${JSON.stringify(ledger, null, 1)}\n`);
            done++;
        }
        catch (e) {
