...
function parseSlate(slate) {
    ...
    if (by)
        return { title: by[1].trim(), voice: by[2].trim() };
...
function titleCase(s) {
    ...
    return words
        ...
        if (i > 0 && i < words.length - 1 && SMALL.has(lower.replace(/[^a-z]/g, "")))
            return lower;
 ...
 */
function voiceConflict(slateVoice, rowVoice) {
    ...
    const anon = (t) => t.includes("unknown") || t.includes("anonymou");
    if (anon(a) || anon(b))
        return false;
...
async function main() {
    ...
    for (const t of timings) {
        ...
        const ranked = rows
            ...
            s: voiceConflict(voice, r.voice)
                ? 0
                : Math.max(score(title, r.title), score(title, r.canonical_filename), 0.95 * contains(r.title, opening, idf)),
        }))
            .filter((c) => c.s > 0)
            .sort((a, b) => b.s - a.s);
    ...
    for (const t of timings) {
        ...
        if (t.error) {
            plan.push({ ...base, note: `timing failed: ${t.error}` });
            continue;
        }
        if (!title) {
    ...
    const line = (p) => `  ${(p.alohaAt == null ? "  ?  " : p.alohaAt.toFixed(2) + "s").padStart(7)}  ${p.file.padEnd(16)} ${p.renameTo ?? p.note ?? ""}`;
    ...
    (0, node_fs_1.writeFileSync)(`${OUT}.csv`, `${csv}\n`);
    console.log(`\n  plan -> ${OUT}.json + .csv`);
    if (!APPLY) {
    ...
    for (const p of [...reshoots, ...fresh]) {
        if (!p.renameTo)
            continue;
        const from = (0, node_path_1.join)(DIR, p.file);
        const to = (0, node_path_1.join)(DIR, p.renameTo);
        if ((0, node_fs_1.existsSync)(to)) {
            console.log(`  exists, skipping: ${p.renameTo}`);
            continue;
        }
