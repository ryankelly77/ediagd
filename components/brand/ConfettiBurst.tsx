...
/**
 * The celebration burst, shared by every "you earned something" moment.
 *
 * Extracted from BadgeCelebration UNCHANGED — same piece count, same maths,
 * same keyframes. It was tuned carefully and shouldn't be re-derived per
 * screen; two copies would drift.
 *
 * WHY IT'S BUILT THIS WAY — an earlier version looked mechanical for two
...
export function ConfettiBurst({ topOffset = 52 }: { topOffset?: number }) {
  ...
  return (
    <>
      <div
        aria-hidden="true"
        className="ediagd-confetti-burst pointer-events-none absolute left-1/2 h-0 w-0"
        style={{ top: topOffset }}
      >
        {pieces.map((p, i) => (
